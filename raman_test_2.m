...
for i = 1:500
    ...
    times(1,i) = toc;
    tic
    interpData2 = interpDTalt(rayGd.DT,rayGd.valsNe,pointsList);
    times(2,i) = toc;
    disp(i)
end
plot(pointNum,times(1,:),pointNum,times(2,:))
legend("original","quadtree")
...
function VqLin = interpDTalt(DT,ValsNe,PqList)
    VqLin = zeros([size(PqList,1),1]);
    for i=1:size(PqList,1)
        [ti,bc] = pointLocationQuadTree(DT,PqList(i,:));
        triVals = ValsNe(DT(ti,:));
        VqLin(i) = dot(bc',triVals')';
    end
end
